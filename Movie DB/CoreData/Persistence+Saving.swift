...
extension PersistenceController {
    ...
    static func saveContext(file: String = #file, line: Int = #line) {
        Logger.coreData.debug("Saving Core Data view context \(file):\(line)")
        shared.saveContext()
    }
    ...
    static func saveContext(_ context: NSManagedObjectContext, file: String = #file, line: Int = #line) async {
        Logger.coreData.debug(
            ...
            "Saving Core Data context from \(file):\(line). Parent: \(context.parent?.description ?? "nil", privacy: .public)"
        )
        Logger.coreData.info("Saving Core Data context with name: \(context.name ?? "unknown", privacy: .public)")
    ...
    private static func saveContexAndWait(_ context: NSManagedObjectContext) {
        Logger.coreData.debug("Starting Core Data save...")
